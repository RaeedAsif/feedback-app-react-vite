...
import { getAccessToken, listFeedback } from "../../services/api"
import { CardWrapper } from "./style"
import moment from 'moment';
import { useTable, usePagination } from 'react-table';
import { Table } from "../../components/Table"

export const ListFeedbackTable:React.FC = () =>{
    const navgate = useNavigate()
    const [page, setPage] = useState(1)
    const [type, setType] = useState("All")
    const { isLoading, data, error, refetch} = useQuery('listFeedback', ()=>listFeedback(page,type));
    ...
    const getPageNumbersDisplay = (count:number) => {
        const displayButton = []
        for(let i=0; i<Math.ceil(count/10); i++){
            displayButton.push( <Button key={i} text={`${i+1}`} onClick={()=>{
                    setPage(i+1)
            }}/>)
        }
    ...
    const navigateToCreateFB = () => navgate(PATHS.CREATE_FB)
    ...
    let feedbacks:any = []
    if (data?.content){
        feedbacks = data?.content.map((feedback)=>{
            const date = moment(feedback.date).format("DD MMM YYYY")
            const time = moment(feedback.date).format("HH:mm:ss")
        ...
        <CardWrapper>
            <div className="card_header">
                ...
                <div className="header_button">
                    <button onClick={()=>navigateToCreateFB()}>
                        <svg fill="#000000" viewBox="0 0 24 24" id="create-note" data-name="Line Color" xmlns="http://www.w3.org/2000/svg" className="icon line-color">
                            <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                            <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                            <g id="SVGRepo_iconCarrier">
                                <path id="secondary" d="M20,4a2.09,2.09,0,0,0-2.95.12L10.17,11,9,15l4-1.17L19.88,7A2.09,2.09,0,0,0,20,4Z" style={{fill: "none", stroke: "#7878bd", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2}}></path>
                                <path id="primary" d="M12,3H4A1,1,0,0,0,3,4V20a1,1,0,0,0,1,1H20a1,1,0,0,0,1-1V12" style={{fill: "none", stroke: "#7878bd", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2}}></path>
                            </g>
                        </svg>
                    </button>
                </div>
            ...
            <div className="card_body">
                <Table columns={columns} data={feedbacks} isLoading={isLoading} buttonText="add a new feedback" onClick={()=>navigateToCreateFB()}/>
            </div>
            <div className="card_foot">
                <div className="foot_buttons">
                    {
                        data?.count&&getPageNumbersDisplay(data.count)
                    }
                </div>
            </div>
        </CardWrapper>
